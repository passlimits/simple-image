...
class h {
  ...
  render() {
    ...
    return i.dataset.placeholder = "Enter a caption", t.appendChild(e), this.data.url && (o.src = this.data.url), o.onload = () => {
      ...
      console.log("Failed to load an image", s);
  ...
  onPaste(t) {
    switch (t.type) {
      case "tag": {
        ...
        e instanceof HTMLImageElement && (this.data = {
          url: e.src
        });
        break;
      }
  ...
  _acceptTuneView() {
    this.tunes.forEach((t) => {
      this.nodes.imageHolder && this.nodes.imageHolder.classList.toggle(this.CSS.imageHolder + "--" + t.name.replace(/([A-Z])/g, (e) => `-${e[0].toLowerCase()}`), !!this.data[t.name]), t.name === "stretched" && this.api.blocks.stretchBlock(this.blockIndex, !!this.data.stretched);
    });
  }
}
export {
